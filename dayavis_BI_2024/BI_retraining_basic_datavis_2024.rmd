...
барплот с двумя категориями - две другие переменные
```{r}
ggplot(hogwarts)+
...
по оси у, мальчики семикурсники, точки по факультетам
```{r}
hogwarts |> 
...
```{r}
ggplot(hogwarts)+
  geom_density(aes(x = result), fill = "aquamarine", colour ="black", bins = 20)+
  theme_bw()+
  theme_custom
```
